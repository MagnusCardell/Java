...
public class HttpParser {
  ...
  /** raw content of an HTTP request, for file I/O;  null unless byte-array 
  constructor called with that HTTP request. */
  ...
  /** Byte-array constructor 
      ...
      @sc Initializes state variables including <code>reader</code>, including 
      contentBytes, after removing any extraneous whitespace from header lines 
      (only) of the raw HTTP request */

      public HttpParser(byte [] buff, int offset, int len) {
    // remove any whitespace garbage from ends of header lines
        String str = new String(buff, offset, len, StandardCharsets.UTF_8);
    //System.out.println("str: " + str);
    int prevNL = -10;  // index of most recent prior newline, or -10 if none
    int nextNL = str.indexOf("\n");
    String thisLine = str.substring(0, nextNL).trim();
    String headers = "";
    /* invar - headers contains cleaned-up header lines encountered in first
       prevNL+1 characters of str, each line with trailing newlines AND 
       nextNL is index of first newline in str after str.charAt(prevNL) AND 
       thisLine is trimmed version of input line ending at str.charAt(nextNL)*/
       while (nextNL != -1 && !thisLine.equals("")) {
        headers += thisLine + "\n";
        prevNL = nextNL;
        nextNL = str.indexOf("\n", prevNL+1);
        if (nextNL != -1)
          thisLine = str.substring(prevNL+1, nextNL).trim();
      }
      if (nextNL != -1) {
      // empty line after headers encountered
        headers += "\n";
        contentBytes = str.substring(nextNL+1).getBytes();      
      }
      str = headers + str.substring(nextNL+1);
    //System.out.println("str: " + str);
    // whitespace garbage removed from header lines only, and 
    // contentBytes assigned to raw bytes of body if complete header found
...
public String getURLBase() {
  ...
  /** retrieve content of original HTTP request 
      @return Unparsed content field of the HTTP request, or 
      null if newline not detected at end of header */
