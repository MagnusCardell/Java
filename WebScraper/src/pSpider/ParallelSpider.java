...
public class ParallelSpider implements Runnable {
	...
	public void run() {
		...
		while (urlCount <= maxUrls) {  // each thread does a certain amount of 'work'
			String link = sharedData.getWork().poll(); //same as b4 but threadsafe
			if (link == null) {
				break;
			}
			
			if(!sharedData.getFinished().contains(link)){//shared resources
				processPage(link); //same as b4
				sharedData.getFinished().add(link);
			}
	...
	public void processPage(String url) {
		...
		for (String url2 : helper.extractLinks(url, html)) {
			...
			if (!helper.isImage(url2)) { //if no image
				sharedData.getUrlCounter().countWord(url2); //thread counts link
				sharedData.getWork().add(url2); //add link to work list
			}
		}
	}
